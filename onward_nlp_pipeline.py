...
### ANX OBSERVATIONS / CLEANSING ACTIONS:
#remove "concerns about" at start
# "uncertainty about " ???
# remove "concerns" at end
# remove "frustration with"
# remove "issues" (begining and end)
#covid == covid19 == covid 19 ?
#time == times
#stopword removal?
#lemtisation?  (S at the end of a word?)
#replace  " s " with ""

### HAP OBSERVATIONS / CLEANSING ACTIONS:
#stopword removal?
#lemtisation?  (S at the end of a word?)
#replace  " s " with ""

## EMOTIONS OBSERVATIONS
#lemetisation? heartbreak/heartbroken	determination/determined	frustration/frustrated    nervousness/nervous	indifferent/indifference
#sorrowful/sorrow	nauseous/nausea    defensiveness/defensive     overwhelm/overwhelmed	reassurance/reassured	
# ?? remove -ness from end of word ??
...
# ## Exploration of seansonal trends
# exploration of seasonal trend, calander month agregation ridge plot: https://python-graph-gallery.com/ridgeline-graph-seaborn/
...
# ## TF-IDF for 3 chronoclogial Periods
...
#path = 'E:/twitter_analysis_data/categorical_data_review/hap_anx_theme_counts/'
#hap_theme_groupby_counts.to_csv(path + 'hap_theme_groupby_counts.csv', index=False)
#anx_theme_groupby_counts.to_csv(path + 'anx_theme_groupby_counts.csv', index=False)
...
#path = 'E:/twitter_analysis_data/categorical_data_review/hap_anx_theme_counts/'
#hap_theme_counts = pd.read_csv(path + 'hap_theme_groupby_counts.csv')
#anx_theme_counts = pd.read_csv(path + 'anx_theme_groupby_counts.csv')
...
def stat_analysis_for_theme(
    ...
    # logisitc regression and GEE modeling of periods, with T-est and effect size
...
# ## Month on Month TF IDF analysis of categoricla variables
# ---
...
glc_df_sorted = glc_df.sort_values(by=['year', 'month_num'], ascending=[True, True])
odered_unqie_mnth_yr_text_list = glc_df_sorted['mnth_yr_text'].unique()


# In[ ]:


odered_unqie_mnth_yr_text_list
...
for mnth in odered_unqie_mnth_yr_text_list:
    print(mnth)
    mnth_df = glc_df_sorted[glc_df_sorted['mnth_yr_text'] == mnth]
    val_list = mnth_df
    print("List length: " + str(len(val_list)) + " List unique strings: " + str(len(set(val_list))))
...
def tf_idf_with_names(corpus, doc_names):
    """
    Calculates TF-IDF scores for a corpus of documents, where each document is a list of strings.
    Input arguments:
        corpus: A list of lists, where each inner list represents a document and contains strings as terms.
        doc_names: A list of strings, where each string is the name of the corresponding document in the corpus.
    Output Returned:
        A pandas DataFrame with one column per document and one row per term, containing the TF-IDF scores.
    """
    if len(corpus) != len(doc_names):
        raise ValueError("Number of documents and document names must match.")
    ...
    tf_idf_scores = OrderedDict()
    for doc_idx, doc in enumerate(sorted_doc_names):
        doc_name = doc_names[doc_idx]
        doc_len = len(doc)
        tf_idf_scores[doc_name] = {}
        for term in set(doc):
            tf = doc.count(term) / doc_len
            idf = math.log(len(corpus) / doc_freqs[term]) + 1
            tf_idf_scores[doc_name][term] = tf * idf
    
    # create pandas DataFrame
    df = pd.DataFrame.from_dict(tf_idf_scores, orient='index')
    return df
...
def return_tfidf_df(cat_df, cat_col):
    #sort input df
    cat_df_sorted = cat_df.sort_values(by=['year', 'month_num'], ascending=[True, True])

    
    # initalise doc name list
    mnth_doc_name_list = cat_df_sorted['mnth_yr_text'].unique()
    print(mnth_doc_name_list)
    corpus = []
    for mnth in mnth_doc_name_list:
        print(mnth)
        df_for_doc = cat_df[cat_df['mnth_yr_text'] == mnth]
        doc_list = df_for_doc[cat_col].to_list()
        corpus.append(doc_list)
        #print(str(len(corpus)))

    print(mnth_doc_name_list)
    print(str(len(mnth_doc_name_list)))
    print(str(len(corpus)))
    
    tf_idf_df = tf_idf_with_names(corpus, mnth_doc_name_list)
...
glc_mnt_tf_idf_df = return_tfidf_df(glc_df, 'glc_topics')
...
hap_top_mnth_tf_idf_df = return_tfidf_df(hap_top_df, 'hap_topics')


# In[ ]:


hap_top_mnth_tf_idf_df
...
hap_top_mnth_tf_idf_df = return_tfidf_df(hap_top_df, 'hap_topics')
anx_top_mnth_tf_idf_df = return_tfidf_df(anx_top_df, 'anx_topics')
emo_mnth_tf_idf_df = return_tfidf_df(emo_df, 'emotions')
glc_mnth_tf_idf_df = return_tfidf_df(glc_df, 'glc_topics')
...
def tf_idf_with_names(corpus, doc_names):
    ...
    # Calculate TF-IDF scores using OrderedDict
    tf_idf_scores = OrderedDict()
    for doc_idx, doc in enumerate(doc_names):
        doc_name = doc_names[doc_idx]        
        doc_len = len(corpus[doc_idx])
        tf_idf_scores[doc_name] = {}
        for term in set(corpus[doc_idx]):        
            tf = corpus[doc_idx].count(term) / doc_len
            idf = math.log(len(corpus) / doc_freqs[term]) #+1 smoothing not applied, to disregard very common terms
            tf_idf_scores[doc_name][term] = tf * idf
            if term == "community connections":
                print(doc_name)
                print(term)
                print("TF: " + str(tf))
                print("IDF: " + str(idf))
                print("TF IDF: " + str(tf * idf))
                
    # Create pandas DataFrame
    df = pd.DataFrame.from_dict(tf_idf_scores, orient='index')
    # Ensure the DataFrame index is sorted in order of sorted_doc_names
    df = df.reindex(doc_names)
    return df

def return_tfidf_df(cat_df, cat_col):
    # Sort input df
    cat_df_sorted = cat_df.sort_values(by=['year', 'month_num'], ascending=[True, True])
    # Initialise doc name list
    mnth_doc_name_list = cat_df_sorted['mnth_yr_text'].unique()
    corpus = []
    for mnth in mnth_doc_name_list:
        df_for_doc = cat_df_sorted[cat_df_sorted['mnth_yr_text'] == mnth]
        doc_list = df_for_doc[cat_col].to_list()
        corpus.append(doc_list)

    # Generate TF-IDF DataFrame with sorted document names
    tf_idf_df = tf_idf_with_names(corpus, mnth_doc_name_list)
    tf_idf_df = tf_idf_df.T
    return tf_idf_df
...
tf_idf_path = 'E:/twitter_analysis_data/categorical_data_review/tf_idf by month outputs/'

hap_top_mnth_tf_idf_df = return_tfidf_df(hap_top_df, 'hap_topics')
hap_top_mnth_tf_idf_df.to_csv(tf_idf_path + 'hap_top_mnth_tf_idf_df_csv.csv', index=True)
del hap_top_mnth_tf_idf_df

anx_top_mnth_tf_idf_df = return_tfidf_df(anx_top_df, 'anx_topics')
anx_top_mnth_tf_idf_df.to_csv(tf_idf_path + 'anx_top_mnth_tf_idf_df_csv.csv', index=True)
del anx_top_mnth_tf_idf_df

emo_mnth_tf_idf_df = return_tfidf_df(emo_df, 'emotions')
emo_mnth_tf_idf_df.to_csv(tf_idf_path + 'emo_mnth_tf_idf_df_csv.csv', index=True)
del emo_mnth_tf_idf_df

glc_mnth_tf_idf_df = return_tfidf_df(glc_df, 'glc_topics')
glc_mnth_tf_idf_df.to_csv(tf_idf_path + 'glc_mnth_tf_idf_df_csv.csv', index=True)
del glc_mnth_tf_idf_df
...
tf_idf_path = 'E:/twitter_analysis_data/categorical_data_review/tf_idf by month outputs/'
hap_top_mnth_tf_idf_df = pd.read_csv(tf_idf_path + 'hap_top_mnth_tf_idf_df_csv.csv', index_col=0)
anx_top_mnth_tf_idf_df = pd.read_csv(tf_idf_path + 'anx_top_mnth_tf_idf_df_csv.csv', index_col=0)
emo_mnth_tf_idf_df = pd.read_csv(tf_idf_path + 'emo_mnth_tf_idf_df_csv.csv', index_col=0)
glc_mnth_tf_idf_df = pd.read_csv(tf_idf_path + 'glc_mnth_tf_idf_df_csv.csv', index_col=0)
...
hap_top_mnth_tf_idf_df.columns


# In[ ]:


def analysis_of_tf_idf_dataset(df, count):
    top_df_list = []
    for col in list(df.columns): 
        month_df = df[[col]]
        month_df_sorted = month_df.sort_values(by=col, ascending=False)
        month_df_top = month_df_sorted.head(count)
        top_list = month_df_top.index.tolist()
        top_dict = {col : top_list}
        top_df_list.append(top_dict)
    top_df = pd.DataFrame([(list(d.keys())[0], list(d.values())[0]) for d in top_df_list], columns=['Month', 'Topics'])
    return top_df
...
n = 200
hap_mnth_tfidf_top = analysis_of_tf_idf_dataset(hap_top_mnth_tf_idf_df, n)


# In[ ]:


hap_mnth_tfidf_top = hap_mnth_tfidf_top.rename(columns={'Month':'month_year'})
split_month_year = hap_mnth_tfidf_top['month_year'].str.split(' ', expand=True)
hap_mnth_tfidf_top  = pd.concat([hap_mnth_tfidf_top , split_month_year], axis=1)
hap_mnth_tfidf_top.rename(columns={0: 'month', 1: 'year'}, inplace=True)
hap_mnth_tfidf_top
...
hap_mnth_tfidf_top_exp = hap_mnth_tfidf_top.explode('Topics', ignore_index=True)
hap_mnth_tfidf_top_exp = hap_mnth_tfidf_top_exp.rename(columns={'Month':'month_year'})
split_month_year = hap_mnth_tfidf_top_exp['month_year'].str.split(' ', expand=True)
hap_mnth_tfidf_top_exp  = pd.concat([hap_mnth_tfidf_top_exp , split_month_year], axis=1)
hap_mnth_tfidf_top_exp.rename(columns={0: 'month', 1: 'year'}, inplace=True)

#hap_mnth_tfidf_top_exp = hap_mnth_tfidf_top_exp[['month', 'year']] = hap_mnth_tfidf_top_exp['month_year'].str.split(' ', expand=True)


# In[ ]:


hap_mnth_tfidf_top_exp
...
month_order = hap_mnth_tfidf_top['month_year'].tolist()
month_order
...
#re_tfidf_by_month_hap = 

df_list = []
for cal_mnth in hap_mnth_tfidf_top['month'].unique():
    #print(cal_mnth)
    df_temp = hap_mnth_tfidf_top[hap_mnth_tfidf_top['month'] == cal_mnth]
    doc_names_list = list(df_temp['month_year'].unique()) 
    #print(doc_names_list)
    list_of_lists = df_temp['Topics'].tolist()
    #print(list_of_lists)
    tfidf_df = tf_idf_with_names(list_of_lists, doc_names_list)
    tfidf_df = tfidf_df.T
    output_df = analysis_of_tf_idf_dataset(tfidf_df, 10)
    df_list.append(output_df)
df_list_df = pd.concat(df_list, ignore_index=True)
reworked_hap_mnth_tfidf_top = df_list_df.rename(columns={'Topics':'topics_reworked'})



reworked_hap_mnth_tfidf_top['Month'] = pd.Categorical(reworked_hap_mnth_tfidf_top['Month'], categories=month_order, ordered=True)
reworked_hap_mnth_tfidf_top = reworked_hap_mnth_tfidf_top.sort_values('Month').reset_index(drop=True)


# In[ ]:


reworked_hap_mnth_tfidf_top = reworked_hap_mnth_tfidf_top.rename(columns={'topics_reworked':'topics_reworked_10'})
reworked_hap_mnth_tfidf_top['topics_reworked_5'] = reworked_hap_mnth_tfidf_top['topics_reworked_10'].apply(lambda x: x[:5] if isinstance(x, list) else [])


# In[ ]:


reworked_hap_mnth_tfidf_top


# In[ ]:


##### MONTH ON MONTH 63 - 12 X 5 WORLDCLOUD GRID #####


# In[ ]:


for x in reworked_hap_mnth_tfidf_top['topics_reworked_5']:
    print(x)
...
# Sample DataFrame with 63 rows (assuming your actual dataframe)
df = reworked_hap_mnth_tfidf_top

# Create a 12x6 grid of subplots
fig, axes = plt.subplots(nrows=12, ncols=6, figsize=(20, 28))

# Iterate through columns and rows
for col in range(6):
    for row in range(12):
        idx = col * 12 + row  # Calculate the index from col-row grid position

        if idx < len(df):  # Ensure within dataframe row count
            word_list = df.iloc[idx]['topics_reworked_5']  # Extract word list from dataframe
            words_string = " ".join(word_list)  # Convert list to space-separated string
            
            # Generate word cloud
            wordcloud = WordCloud(width=420, height=300, colormap='Wistia', background_color='white').generate(words_string)
    ...
    #).generate_from_frequencies(hap_topic_count_dict)
            ...
            # Plot word cloud in respective position
            axes[row, col].imshow(wordcloud, interpolation='bilinear')
            #axes[row, col].set_title(df.iloc[idx]['Category'], fontsize=10)
            axes[row, col].axis('off')
        else:
            # Hide the bottom 9 subplots in the 6th column
            if col == 5 and row >= 3:
                axes[row, col].axis('off')
...
# # SPECIFIC TOPIC GROUPS OVER TIME
# ---
# grouping most preventent 
# filtering data to each of those groups and visualising mean month on month value
# wordcloud as backgorund of linechart: https://python-graph-gallery.com/web-stacked-charts/
# 
# 

# # HAPPNIESS AND ANXIETY TOPIC ASSOCIATIONS
# ---
# Are certain drivers of happiness associated with certain drivers of anxiety, and vice versa for individuals?
# Analysis to drive insight into the profile of resident users based on what makes them happy and anxious.
...
hap_anx_rel_df = df[['concat_key', 'hap_topics', 'anx_topics']]


# In[ ]:


hap_anx_rel_df.sample(3)


# In[ ]:


hap_anx_rel_df.loc[:, 'combinations'] = hap_anx_rel_df.apply(lambda row: list(product(row['hap_topics'], row['anx_topics'])), axis=1)

#df.loc[:, 'Combinations'] = df.apply(lambda row: [(x, y) for x, y in product(row['Column1'], row['Column2'])], axis=1)
...
hap_anx_rel_df_explode = hap_anx_rel_df.explode('combinations')


# In[ ]:


hap_anx_rel_df_explode
...
# ### Which topics are associated whith higher or lower levels of Anxiety / Happiness?
...
# ### Normalising data at user level
# ---
# 
# 
...
emotion_counts_dict = unique_item_occurence_counts(df['emotions_cleansed'])
...
wordcloud = WordCloud(
    ...
    background_color='white', 
    colormap='viridis',
).generate_from_frequencies(emotion_counts_dict)

# Display the word cloud
plt.figure(figsize=(10, 5))
plt.imshow(wordcloud, interpolation='bilinear')
plt.axis('off')
plt.title("Emotions", fontsize=13, loc='left')
plt.show()
...
# # ANALYSIS OF GOOD LIFE CAMDEN TOPICS
# ----

# In[ ]:


glc_topics_counts_dict = unique_item_occurence_counts(df['glc_topics_cleansed'])


# In[ ]:


glc_topics_counts_dict


# In[ ]:


['cultural identity', 'local spaces', \
...
wordcloud = WordCloud(
    ...
    background_color='white', 
    colormap='viridis',
).generate_from_frequencies(glc_topics_counts_dict)
...
# ## Analysis of Happiness and Anxiety Scores
# ---
...
hap_score_cols = ['concat_key', 'user_id', 'year', 'month_num', 'month_abbr',
                  'month_name', 'mnth_yr_text', 'hap_lvl', 'hap_conf', 'hap_topics']
hap_score_df = df[hap_score_cols].copy()


# In[ ]:


anx_score_cols = ['concat_key', 'user_id', 'year', 'month_num', 'month_abbr',
                  'month_name', 'mnth_yr_text', 'anx_lvl', 'anx_conf', 'anx_topics']
anx_score_df = df[hap_score_cols].copy()


# In[ ]:


hap_anx_score_cols = ['concat_key', 'user_id', 'year', 'month_num', 'month_abbr','month_name',
                      'mnth_yr_text', 'hap_lvl', 'hap_conf', 'hap_topics', 'anx_lvl', 'anx_conf', 'anx_topics']
hap_anx_score_df = df[hap_anx_score_cols].copy()


# In[ ]:


path = 'E:/twitter_analysis_data/numeric_data_review/hap_anx_base_data/'
hap_score_df.to_csv(path + 'hap_score_df_csv.csv', index=False)
anx_score_df.to_csv(path + 'anx_score_df_csv.csv', index=False)
hap_anx_score_df.to_csv(path + 'hap_anx_score_df_csv.csv', index=False)
...
hap_anx_score_stat_summary = hap_anx_score_df.describe()
hap_anx_score_stat_summary = hap_anx_score_stat_summary[['hap_lvl', 'hap_conf', 'anx_lvl', 'anx_conf']]


# In[ ]:


hap_anx_score_stat_summary
...
print("\033[1mSUMMARY OF HAPPINESS AND ANXIETY SCORES\033[0m\n")
print(hap_anx_score_stat_summary[['hap_lvl', 'anx_lvl']])
plt.figure(figsize=(11, 2))
plt.boxplot(hap_anx_score_df[['hap_lvl', 'anx_lvl']], tick_labels=['Happiness', 'Anxiety'], vert=False,)
plt.title('Happiness and Anxiety Level Scores')
plt.grid(True)
plt.show()
...
print("\033[1mSUMMARY OF HAPPINESS AND ANXIETY SCORE CONFIDENCE LEVELS\033[0m\n")
print(hap_anx_score_stat_summary[['hap_conf', 'anx_conf']])
plt.figure(figsize=(11, 2))
plt.boxplot(hap_anx_score_df[['hap_conf', 'anx_conf']], tick_labels=['Happiness \n Confidence', 'Anxiety \n Confidence'], vert=False,)
plt.title('Happiness and Anxiety Level Score Confidence')
plt.grid(True)
plt.show()
...
hap_anx_score_df.columns


# In[ ]:


## LEVEL / CONFIDENCE RELATIONSHIP 
...
## Happiness Score versus Happiness Score Confidence Level
# define variables
x = hap_anx_score_df['hap_lvl']
y = hap_anx_score_df['hap_conf']
# calaculate and print correlation measures
correlation = np.corrcoef(x, y)[0, 1]
...
print(f'Linear Regression: Slope: {slope:.2f}, Intercept: {intercept:.2f}, R-squared: {r_value**2:.2f}')
plt.figure(figsize=(11, 3))
plt.scatter(x, y, s=10, c='#FFDB58', alpha=0.1, marker='o')
plt.xlabel('Score')
plt.ylabel('Confidence')
plt.title('Happiness Score versus Confidence')
plt.grid(True)
m, b = np.polyfit(x, y, 1)
plt.plot(x, m*x + b, color='teal', alpha=0.4, label='Trend Line')
plt.show()
...
## Happiness Score versus Anxiety Score Confidence Level
# define variables
x = hap_anx_score_df['anx_lvl']
y = hap_anx_score_df['anx_conf']
# calaculate and print correlation measures
correlation = np.corrcoef(x, y)[0, 1]
print(f'Pearson Correlation: {correlation:.2f}')
spearman_corr, _ = spearmanr(x, y)
...
print(f'Linear Regression: Slope: {slope:.2f}, Intercept: {intercept:.2f}, R-squared: {r_value**2:.2f}')
plt.figure(figsize=(11, 3))
plt.scatter(x, y, s=10, c='grey', alpha=0.1, marker='o')
plt.xlabel('Score')
plt.ylabel('Confidence')
plt.title('Anxiety Score versus Confidence')
plt.grid(True)
m, b = np.polyfit(x, y, 1)
plt.plot(x, m*x + b, color='teal', alpha=0.4, label='Trend Line')
plt.show()
...
hap_score_mnths = hap_anx_score_df.sort_values(by=['year', 'month_num'], ascending=True)
mnths_in_order = hap_score_mnths['mnth_yr_text'].unique()

hap_score_mnths = hap_score_mnths.groupby('mnth_yr_text')['hap_lvl'].apply(list).reset_index()
hap_score_mnths['mnth_yr_text'] = pd.Categorical(hap_score_mnths['mnth_yr_text'], categories=mnths_in_order, ordered=True)
hap_score_mnths = hap_score_mnths.sort_values('mnth_yr_text').reset_index(drop=True)

data = list(hap_score_mnths['hap_lvl'])
labels = mnths_in_order
...
# Add title and labels
plt.title("Month on Month Happiness Levels")
plt.ylabel("Values")
plt.grid(True)
...
# add confidence level filter ####
anx_score_mnths = hap_anx_score_df.sort_values(by=['year', 'month_num'], ascending=True)
mnths_in_order = anx_score_mnths['mnth_yr_text'].unique()

anx_score_mnths = anx_score_mnths.groupby('mnth_yr_text')['anx_lvl'].apply(list).reset_index()
anx_score_mnths['mnth_yr_text'] = pd.Categorical(anx_score_mnths['mnth_yr_text'], categories=mnths_in_order, ordered=True)
anx_score_mnths = anx_score_mnths.sort_values('mnth_yr_text').reset_index(drop=True)

data = list(anx_score_mnths['anx_lvl'])
labels = mnths_in_order
...
# Add title and labels
plt.title("Anxiety level by month")
plt.ylabel("Values")
plt.grid(True)
...
### MEAN VALUE PER MONTH - LIEN CHART ###
...
## create dataframe of average scores per month
hap_score_mean_mnths = hap_anx_score_df.sort_values(by=['year', 'month_num'], ascending=True)
mnths_in_order = hap_score_mnths['mnth_yr_text'].unique()
hap_score_mean_mnth = hap_anx_score_df[['mnth_yr_text', 'hap_lvl']].copy()

### >>>>> filter to confidence level ####
hap_score_mean_mnth = hap_score_mean_mnth.groupby('mnth_yr_text')['hap_lvl'].agg(['mean',     # mean
                                                                            'median',   # median
                                                                            lambda x: x.mode().iloc[0],  # mode
                                                                            'std'])
hap_score_mean_mnth.columns = ['mean', 'median', 'mode', 'std_dev']
hap_score_mean_mnth.index = pd.Categorical(hap_score_mean_mnth.index, categories=mnths_in_order, ordered=True)
hap_score_mean_mnth = hap_score_mean_mnth.sort_index().reset_index()
hap_score_mean_mnth = hap_score_mean_mnth.rename(columns={'index': 'mnth_yr_text'})

mean_lin = list(hap_score_mean_mnth['mean'])
med_lin = list(hap_score_mean_mnth['median'])
mod_lin = list(hap_score_mean_mnth['mode'])
std_lin = list(hap_score_mean_mnth['std_dev'])

# abreviate month labels
abr_mnths = []
for mnth in mnths_in_order:
    mnth_arb = mnth[:3]
    yr_abr =  mnth[-2:]
    abr = mnth_arb + '\n' + yr_abr
    abr_mnths.append(abr)

# set x axis
x = abr_mnths

# plot
fig, ax = plt.subplots(4, 1, figsize=(11, 8), sharex=True, sharey=False)

ax[0].plot(x, mean_lin, color='#FFDB58', marker='o')
ax[0].set_ylabel('Mean')
ax[0].tick_params(axis='x', which='both', bottom=False, top=False, labelbottom=False)

ax[1].plot(x, med_lin, color='#FFDB58', marker='s')
ax[1].set_ylabel('Median')
ax[1].tick_params(axis='x', which='both', bottom=False, top=False, labelbottom=False)

ax[2].plot(x, mod_lin, color='#FFDB58', marker='s')
ax[2].set_ylabel('Mode')
ax[2].tick_params(axis='x', which='both', bottom=False, top=False, labelbottom=False)

ax[3].plot(x, std_lin, color='#FFDB58', marker='s')
ax[3].set_ylabel('Standard Deviation')
#ax[3].tick_params(axis='x', which='both', bottom=False, top=False, labelbottom=False)

ax[3].set_xticklabels(labels=abr_mnths, fontsize=5)

plt.suptitle('Average Happiness Level Scores Month on Month', fontsize=16)
...
## create dataframe of average scores per month
anx_score_mean_mnths = hap_anx_score_df.sort_values(by=['year', 'month_num'], ascending=True)

mnths_in_order = hap_score_mnths['mnth_yr_text'].unique()

anx_score_mean_mnth = hap_anx_score_df[['mnth_yr_text', 'anx_lvl']].copy()

anx_score_mean_mnth = anx_score_mean_mnth.groupby('mnth_yr_text')['anx_lvl'].agg(['mean',     # mean
                                                                            'median',   # median
                                                                            lambda x: x.mode().iloc[0],  # mode
                                                                            'std'])
anx_score_mean_mnth.columns = ['mean', 'median', 'mode', 'std_dev']
anx_score_mean_mnth.index = pd.Categorical(anx_score_mean_mnth.index, categories=mnths_in_order, ordered=True)
anx_score_mean_mnth = anx_score_mean_mnth.sort_index().reset_index()
anx_score_mean_mnth = anx_score_mean_mnth.rename(columns={'index': 'mnth_yr_text'})

mean_lin = list(anx_score_mean_mnth['mean'])
med_lin = list(anx_score_mean_mnth['median'])
mod_lin = list(anx_score_mean_mnth['mode'])
std_lin = list(anx_score_mean_mnth['std_dev'])
...
ax[0].plot(x, mean_lin, color='grey', marker='o', markersize=3)
ax[0].set_ylabel('Mean')
ax[0].tick_params(axis='x', which='both', bottom=False, top=False, labelbottom=False)

ax[1].plot(x, med_lin, color='grey', marker='o', markersize=3)
ax[1].set_ylabel('Median')
ax[1].tick_params(axis='x', which='both', bottom=False, top=False, labelbottom=False)

ax[2].plot(x, mod_lin, color='grey', marker='o', markersize=3)
ax[2].set_ylabel('Mode')
ax[2].tick_params(axis='x', which='both', bottom=False, top=False, labelbottom=False)

ax[3].plot(x, std_lin, color='grey', marker='o', markersize=3)
ax[3].set_ylabel('Standard Deviation')
#ax[3].tick_params(axis='x', which='both', bottom=False, top=False, labelbottom=False)

ax[3].set_xticklabels(labels=abr_mnths, fontsize=5)

plt.suptitle('Average Anxiety Level Scores Month on Month', fontsize=16)
...
## SCATTER OF ALL SCORES PER MONHT - WITH NOISE TO GITTER WITHIN MONTH
...
sct_tim_cols = ['concat_key', 'user_id', 'year', 'month_num', 'month_abbr', 'month_name', 'mnth_yr_text',
                'hap_lvl', 'hap_conf', 'hap_topics', 'hap_explain', 'anx_lvl', 'anx_conf', 'anx_topics']

scatter_timeline_df = df[sct_tim_cols].copy()


# In[ ]:


scatter_timeline_df.sample(3)


# In[ ]:


# create new dataframe, derived from source data
sct_tim_cols = ['concat_key', 'user_id', 'year', 'month_num', 'month_abbr', 'month_name', 'mnth_yr_text',
                'hap_lvl', 'hap_conf', 'hap_topics', 'hap_explain', 'anx_lvl', 'anx_conf', 'anx_topics']
...
scatter_timeline_df['days_in_month'] = scatter_timeline_df.apply(get_days_in_month, axis=1)

#asign random day in month
def generate_random_day_of_month(row):
    return random.randint(1, row['days_in_month'])
scatter_timeline_df['day'] = scatter_timeline_df.apply(generate_random_day_of_month, axis=1)
scatter_timeline_df['days_in_month'] = scatter_timeline_df.apply(get_days_in_month, axis=1)

#rename month_num
scatter_timeline_df = scatter_timeline_df.rename(columns={'month_num': 'month'})

# construct date
scatter_timeline_df['jittered_date'] = pd.to_datetime(scatter_timeline_df[['year', 'month', 'day']])

# jitter hap score
scatter_timeline_df['hap_lvl_jitter_flt'] = np.random.uniform(-0.49999999999999999, 0.5, size=len(df))
scatter_timeline_df['hap_lvl_jittered'] = scatter_timeline_df['hap_lvl'] + scatter_timeline_df['hap_lvl_jitter_flt']

# jitter anx score
scatter_timeline_df['anx_lvl_jitter_flt'] = np.random.uniform(-0.49999999999999999, 0.5, size=len(df))
scatter_timeline_df['anx_lvl_jittered'] = scatter_timeline_df['anx_lvl'] + scatter_timeline_df['anx_lvl_jitter_flt']
...
x_snap_dates = scatter_timeline_df[['year', 'month', 'mnth_yr_text']]
x_snap_dates = x_first_dates.drop_duplicates(subset=['mnth_yr_text'])
x_snap_dates = x_first_dates.sort_values(by=['year','month'], ascending=True)
x_snap_dates['day'] = 15
x_snap_dates['snap_date'] = pd.to_datetime(x_snap_dates[['year', 'month', 'day']])
x_snap_dates = x_snap_dates['snap_date']
...
dates = scatter_timeline_df['jittered_date']
values = scatter_timeline_df['hap_lvl_jittered']

# momth on month mean
mean_lin = list(hap_score_mean_mnth['mean'])
x_snap_dates = x_snap_dates

# fist of each month


x = dates
x_ordinal = scatter_timeline_df['jittered_date'].map(lambda d: d.toordinal())
y = values

# Perform linear regression (1st-degree polynomial fit)
coefficients = np.polyfit(x_ordinal, y, 1)
polynomial = np.poly1d(coefficients)
# Convert dates for plotting the best fit line
x_fit = np.linspace(x_ordinal.min(), x_ordinal.max(), 100)
y_fit = polynomial(x_fit)
# Convert ordinal numbers back to datetime for plotting
...
plt.scatter(dates, values, color='#FFDB58', marker='o', s=4, alpha=0.15, edgecolors='#FFDB58', linewidths=0.0)
plt.plot(x_snap_dates, mean_lin,  color='white', linestyle='-',linewidth=8, marker='o', markersize=3, label='backgroumd')
plt.plot(x_snap_dates, mean_lin,  color='#FFDB58', linestyle='--',linewidth=1, marker='o', markersize=3, label='Month on Month Mean Values')

# Format the x-axis to show readable dates
plt.gcf().autofmt_xdate()
# Set axis limits
plt.ylim(0, 10)  # Set y-axis range from 5 to 45

# Add labels and title
#plt.xlabel('Date')
#plt.ylabel('Value')
plt.title('Scatter Plot with Dates')

plt.gca().yaxis.set_major_locator(ticker.MaxNLocator(nbins=10))
plt.xticks(rotation=0, ha='left')

# Custom grid using ax object
plt.grid(True, color='gray', linestyle='--', linewidth=0.7, alpha=0.5)
plt.show()
...
# ### Emojis over time
# ---
# TFIDF month on month applied to emojis
# Load user/month tweet block source data
# Extract emojis
# TF - DFI month-to-month 
# visualise top 10 per month, sizing by score
# 
# 
